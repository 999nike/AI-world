...
class UtilityAgent:
    ...
    def act(self, obs: Observation, rng: RNG) -> Action:
        ...
        # E5.9 hard gate: force Library once inquiry live (always attempt build)
        nearest = obs.nearest_settlement or {}
        ...
        if era >= 4 and "inquiry" in subjects and "library" not in types:
            ...
            # On occupied tile: pure move to hunt empty tile (no gather, no road)
            moves = [(1, 0), (0, 1), (-1, 0), (0, -1)]
            dx, dy = moves[rng.randint(0, 3)]
        ...
        if a.type == "build":
            ...
            if b == "road":
                if "inquiry" in subjects:
                    return -8.0  # E5.9: stop road spam after science path opens
