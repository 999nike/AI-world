        ...
        for line in f:
            ...
            if t in KEY_EVENT_TYPES:
                ...
                if b in ("academy", "walls", "irrigation", "library", "foundry",
                         "hall", "command", "lab", "observatory", "temple", "barracks", "workshop"):
                    out.append(ev)
    return out
...
def structure_counts(snap: dict) -> str:
    ...
    order = ["farm", "storage", "hut", "granary", "mine", "road",
             ...
             "irrigation", "library", "foundry", "hall", "command", "lab", "observatory"]
...
def print_grid(grid, tick, summary=None, snap=None, callouts=None):
    ...
    if summary:
        ...
        print(
            ...
            f"R:{m.get('build_lab',0)} V:{m.get('build_observatory',0)}"
        )
    if snap:
    ...
    print("  #=walls ~=irrigation L=library Y=foundry O=hall X=command R=lab V=observatory")
    print()
